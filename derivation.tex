...
\title{Complete Derivation: Equivalence of Equations (F.19) and (F.21)}
\author{Supplementary Material for\\``A Game-Theoretic Approach for Pricing Policy under Urban Rail Mixed Passenger-Freight Transport''}
\date{}
...
\section*{Abstract}
This document provides the complete algebraic derivation demonstrating the equivalence between equations (F.19) and (F.21) in Appendix F of the main paper. The derivation shows that on the boundary of the effective cooperation zone, these two expressions for passenger sensitivity $\phi$ are mathematically equivalent.
...
All other parameters maintain their definitions from the main paper (see Appendix A).

\section*{Derivation}

\subsection*{Step 1: Starting from the Discriminant Condition}
The discriminant for the Nash bargaining solution (Appendix H, equation H.17) is:
\[
...
Taking the positive square root (valid for realistic parameters):
\[
A - F\eta\left(k_1 + (2w + \tau)\phi\right) = 2F\eta\sqrt{\Theta}. \tag{2}
\]

\subsection*{Step 2: Solving for $\phi$ (Equation F.19)}
From equation (2), solve for $\phi$:
\[
\phi = \frac{A}{F\eta(2w + \tau)} - \frac{k_1}{2w + \tau} - \frac{2\sqrt{\Theta}}{2w + \tau}. \tag{F.19}
\]

\subsection*{Step 3: Expressing $A$ in Terms of $\kappa_1$}
First, expand $A$:
\[
...
\subsection*{Step 4: Substituting into Equation (F.19)}
...
\subsection*{Step 5: Equation (F.21) Form}
...
\subsection*{Step 6: Establishing the Equivalence Condition}
...
\subsection*{Step 7: Verifying the Condition Holds on the Boundary}
...
Set this equal to the RHS of equation (2), $2F\eta\sqrt{\Theta}$:
\[
